...
Fetches a token, then calls the **Items** endpoint specifically — some accounts have valid
credentials but no Items permission, which otherwise only surfaced as a failed catalog sync.
- `invalid_credentials` — bad token request
- `permission_denied` — token OK, Items call rejected
